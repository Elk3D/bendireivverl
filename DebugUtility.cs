...
public static class DebugUtility
{
	...
	public static void DebugCone(Transform transform, Color color, float angle = 45f, float range = 1f, float duration = 0f, bool depthTest = true)
	{
		Vector3 position = transform.position;
		Vector3 forward = transform.forward;
		Vector3 vector = transform.forward * range;
		Vector3 up = transform.up;
		Vector3 right = transform.right;
		forward = forward.normalized;
		Vector3 direction = Vector3.Slerp(vector, up, angle / 90f);
		Plane plane = new Plane(-forward, position + vector);
		Ray ray = new Ray(position, direction);
		plane.Raycast(ray, out var enter);
		Debug.DrawRay(position, Vector3.Slerp(vector, up, angle / 90f).normalized * enter, color, duration, depthTest);
		Debug.DrawRay(position, Vector3.Slerp(vector, -up, angle / 90f).normalized * enter, color, duration, depthTest);
		Debug.DrawRay(position, Vector3.Slerp(vector, right, angle / 90f).normalized * enter, color, duration, depthTest);
		Debug.DrawRay(position, Vector3.Slerp(vector, -right, angle / 90f).normalized * enter, color, duration, depthTest);
		DebugCircle(position + vector, forward, color, (vector - direction.normalized * enter).magnitude, duration, depthTest);
	}
