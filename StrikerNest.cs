...
public class StrikerNest : ButcherGangSpawner
{
	[Header("Trigger")]
	[SerializeField]
	...
	private Vector3 m_InitialEnterPosition;

	private Vector3 m_InitialExitPosition;
	...
	{
		m_InitialEnterPosition = m_EnterLocation.position;
		m_InitialExitPosition = m_ExitLocation.position;
		Vector3 position = base.transform.position;
		position.y -= 2f;
		Vector3 end = position;
		end.y -= 50f;
		if (Physics.Linecast(position, end, out var hitInfo, -1, QueryTriggerInteraction.Ignore))
		{
			Vector3 position2 = m_EnterLocation.position;
			position2.y = hitInfo.point.y;
			m_EnterLocation.position = position2;
			Vector3 position3 = m_ExitLocation.position;
			position3.y = hitInfo.point.y;
			m_ExitLocation.position = position3;
			Vector3 position4 = m_EventTrigger.transform.position;
			position4.y = hitInfo.point.y + 5f;
			m_EventTrigger.transform.position = position4;
		}
	}
	...
	private void HandleEventTriggerOnEnter(object sender, EventArgs e)
	{
		...
		position.y = m_EnterLocation.transform.position.y;
		Vector3 normalized = (position - m_EnterLocation.transform.position).normalized;
		...
		m_ActiveSequence.InsertCallback(0.2f, delegate
		{
			...
			position2.y = GameManager.Instance.Player.transform.position.y;
			GameManager.Instance.Player.PlayerMovement.SetPreviouslyGrounded(active: true);
			GameManager.Instance.Player.AddForce((GameManager.Instance.Player.transform.position - position2).normalized * 25f);
			GameManager.Instance.Player.PlayerMovement.SetPreviouslyGrounded(active: true);
		});
		SendOnSpawned();
	}
	...
	private void HandleEnemyOnInitializeOnComplete(object sender, EventArgs e)
	{
		...
		m_Enemy.transform.position = m_EnterLocation.position;
		m_Enemy.Content.transform.position = m_InitialEnterPosition;
		m_ActiveSequence.Insert(0.3f, m_Enemy.Content.transform.DOMove(m_EnterLocation.position, 0.2f).SetEase(Ease.Linear));
	...
	{
		m_Enemy.OnAnimationComplete -= HandleSpawnedEnemyOnAnimationComplete;
		m_Enemy.Content.transform.localPosition = Vector3.zero;
		m_Enemy.transform.position = m_EnterLocation.position;
	...
	private void HandleEnemyOnNodeReached(object sender, EventArgs e)
	{
		...
		m_Enemy.Controller.enabled = false;
		ResetActiveSequence();
		m_ActiveSequence.Insert(0.3f, m_Enemy.transform.DOMove(m_EnterLocation.position, 0.25f));
		m_ActiveSequence.Insert(0.3f, m_Enemy.Content.transform.DOMove(m_InitialExitPosition, 0.25f));
