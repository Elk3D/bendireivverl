...
public class CutsceneDirector : JMonoBehaviour
{
	[SerializeField]
	...
	{
		if (!base.IsDisposed && IsPlaying && !IsComplete && !GameManager.Instance.IsPaused && (m_PlayableDirector.time >= m_PlayableDirector.duration || m_PlayableDirector.state != PlayState.Playing))
		{
			IsComplete = true;
			IsPlaying = false;
	...
	public void Play(Action onComplete = null, Action onPaused = null, Action onStopped = null)
	{
		JDebug.Log("CutsceneDirector :: Play", this, JDebug.JDebugType.Cutscene);
		IsPlaying = true;
		m_PlayableDirector.Play();
